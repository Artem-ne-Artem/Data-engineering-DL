...
select 100+row_number() over(), ship_mode from (select distinct ship_mode from stg.orders ) as a;
--checking
...
select 100+row_number() over(), customer_id, customer_name from (select distinct customer_id, customer_name from stg.orders ) as a;
--checking
...
select 100+row_number() over(), country, city, state, postal_code from (select distinct country, city, state, postal_code from stg.orders ) as a;
----------------------------------------------------------------------------------------------------------------------------------------------------------
--data quality check
...
select 100+row_number() over () as prod_id ,product_id, product_name, category, subcategory, segment from (select distinct product_id, product_name, category, subcategory, segment from stg.orders ) as a;
--checking
...
select		to_char(date,'yyyymmdd')::int as date_id,  
			...
			extract('day' from (date + interval '2 month - 1 day') ) = 29
       		as leap
